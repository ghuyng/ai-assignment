...
def move(board, player, remain_time):
    ...
    move = choose_move_alg1(prev_boards[player], board, player, remain_time)
    if not move:
        return None
    src, des = move
    new_board = board_after_move_and_capturing(src, des, board)
...
def simulate():
    ...
    while get_winner(board) == 0:
        ...
        if player == -1:
            src, des = choose_move_alg0(
                prev_boards[player], deepcopy(board), player, 30.0
            )
        else:
            src, des = choose_move_alg1(
                prev_boards[player], deepcopy(board), player, 30.0
            )
...
def choose_move_alg0(prev_board, board, player, remain_time):
